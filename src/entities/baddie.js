...
export default class Baddie extends EngineObject {
  constructor(player, {sfx, updateScore}) {
    ...
    super(vec2(startPos) , vec2(size), tile(5, 4))
    this.player = player;
    this.sfx = sfx;
  ...
  collideWithObject(o) {
    if (o.name === 'p1' || o.name === 'bullet') {
      ...
      new ParticleEmitter(
        this.pos, 0.2,            // pos, angle
        vec2(this.size.x * 2), .1, 2000, PI, // emitSize, emitTime, emitRate, emiteCone
        tile(7,4),                      // tileInfo
        color, color2,           // colorStartA, colorStartB
        color.scale(1,0), color2.scale(1,0), // colorEndA, colorEndB
        .3, 1, 5, .1, .1,  // time, sizeStart, sizeEnd, speed, angleSpeed
