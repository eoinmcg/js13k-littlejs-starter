...
const setGameOver = (val) => { gameOver = val; music.stop(); };
...
function gameUpdate() {
  ...
  const clicked = keyWasPressed('Space') || keyWasPressed('KeyX') || gamepadWasPressed(2)
  ...
  if (!gameOver && ready && Math.random() > .991) {
...
function gameRender() {
  ...
  if (!ready) {
    ...
    if (flash) font.drawTextScreen('READY?', vec2(center, D.height / 1.5), 1.5, true);
...
function gameRenderPost() {
  ...
  drawTextScreen('LittleJS JS13K Demo', vec2(mainCanvasSize.x/2, mainCanvasSize.y - 15), 15);
